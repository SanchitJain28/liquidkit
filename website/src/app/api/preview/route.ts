...
export async function POST(req: NextRequest) {
  try {
    const slug = req.nextUrl.searchParams.get("component");
    if (!slug) {
      return NextResponse.json({ error: "Missing component param" }, { status: 400 });
    }

    const body = await req.json();
    const customMockData = body.mockData;
    ...
    if (!liquidFile) {
      return NextResponse.json({ error: "No liquid file found" }, { status: 404 });
    }
    ...
    const html = await renderComponent(slug, liquidSource, cssSource, jsSource, customMockData);
